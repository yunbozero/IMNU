...
test('部署：首页内容要和备案口径一致，且不出现金额', () => {
  ...
  assert.match(html, /流浪猫|救助|公益/, '首页应当说明这个项目是做什么的');
  assert.match(html, /小程序/, '应当说明服务于小程序');
  assert.match(html, /仅(用于)?(登记|锁定)[^。，]{0,8}名额/, '应当保留"仅登记名额"这句合规声明');

  // ★ 个人备案的关键：必须声明不是学校/组织官方机构。
  //   自称"官方"而主体是个人，是备案被驳回或事后被注销的常见原因。
  assert.match(html, /自发组织|学生公益/, '应当说明是学生自发组织');
  assert.match(html, /(尚无|非|不是)[^。，]{0,10}官方/, '应当明确声明没有官方身份');
